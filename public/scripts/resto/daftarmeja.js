                ...
                if (response.success == "true"){
                    ...
                    if (response.totaldata == 0){
                        Swal.fire({
                            ...
                            text: "Silahakn buat pesanan terlebih dahulu sebelum melihat detil dari NOTA PESANAN ini",
                            icon: 'error',
                        });
                    }else{
                        Swal.fire({
                            title: "Terjadi Pada QUERY Database",
                            text: response.msg,
                            icon: 'error',
                        });
                    }
    ...
    getCsrfTokenCallback(function() {
        $.ajax({
            ...
            error: function(xhr, status, error) {
                toastr["error"]("DAFTAR LANTAI ERROR : "+ xhr.responseJSON.message);
            }
        });
    });
}
...
function modalubahdatameja(namameja, kodemeja, keterangan, namalantai, waktuawal, waktuakhir, urlgambar){
    ...
    $("#waktuaktif").val((waktuawal == "" ? moment().startOf('day').format('HH:mm') : moment(waktuawal, 'HH:mm:ss').format('HH:mm') ));
    $("#waktuakhir").val((waktuakhir == "" ? moment().format('HH:mm') : moment(waktuakhir, 'HH:mm:ss').format('HH:mm') ));
    $('#modalinformasimeja').modal('show'); 
}
function simpaninformasimeja(){
    kondisi = 1;
    if ($('#kodemeja').is('[readonly]')) {
        kondisi = 0;
     }
    swal.fire({
                    ...
                    success: function (response) {
                        if (response.success == "true"){    
                            ...
                            swal.fire({
                                ...
                                if(result.isConfirmed){ bersihkanmodalinformasimeja()}else{$('#modalinformasimeja').modal('hide'); }
                            })
                        }else{
                            Swal.fire({
...
function cetakulangnota(nomortransaksi,kodeai){
    let nomortransaksinya, routercetaknota;
    if (nomortransaksi == "meja"){
        routercetaknota = "cetaknotapesanan";
        nomortransaksinya = $("#namapemesan").html().substring(
            $("#namapemesan").html().indexOf("[") + 1, 
            $("#namapemesan").html().lastIndexOf("]")
        );
    }else{
        routercetaknota = "cetaknota";
        nomortransaksinya = nomortransaksi
    }
    ...
    swal.fire({
        ...
        icon: 'question',
        text: "Apakah anda ingin mencatak ulang nota dengan KODEPESAN "+nomortransaksinya+" ini ?",
        showCancelButton:true,
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success == "true"){
                            ...
                            getCsrfTokenCallback(function() {
                                $.ajax({
                                    url: baseurljavascript + 'penjualan/'+routercetaknota,
                                    method: 'POST',
                                    dataType: 'json',
                                    data: {
                                        [csrfName]:csrfTokenGlobal,
...
$("#cetakpesanan").on("click", function() {
    cetakulangnota("meja",$("#kodeai").html());
});